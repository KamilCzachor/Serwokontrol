...
if (hamburgerBtn && navMenu) {
  hamburgerBtn.addEventListener("click", function () {
    ...
    setMenuState(hamburgerBtn, navMenu, isOpen);
  });
...
if (stickyHamburgerBtn && stickyNavMenu) {
  stickyHamburgerBtn.addEventListener("click", function () {
    const isOpen = !stickyNavMenu.classList.contains("show");
    setMenuState(stickyHamburgerBtn, stickyNavMenu, isOpen);
  });
...
function initRepresentativesRegionMap() {
  if (window.__serwokontrolRepresentativesMapInitialized) return;
  window.__serwokontrolRepresentativesMapInitialized = true;
  ...
  function smoothScrollToRepresentatives(force = false) {
    ...
    if (force || window.innerWidth <= 992) {
      const scrollTarget =
        representativesSection.querySelector(".representatives_content") ||
        representativesSection.querySelector(".representatives_map_panel") ||
        representativesSection.querySelector(".city_search_box") ||
        representativesSection;

      const targetTop =
        scrollTarget.getBoundingClientRect().top +
        window.pageYOffset -
        Math.max((window.innerHeight - scrollTarget.offsetHeight) / 2, 90) +
        110;
  ...
  function setupAccessibleMapAreas() {
    mapAreas.forEach(function (area) {
      ...
      if (cityName) {
        area.setAttribute(
          "aria-label",
          "Wybierz miasto lub powiat: " + cityName,
        );
      }
  ...
  if (citySearchInput && citySuggestions && representativesSection) {
    ...
    citySearchInput.addEventListener("keydown", function (event) {
      ...
      const firstSuggestion =
        citySuggestions.querySelector("li:not(.is_empty)");
...
(function () {
  ...
  if (mapMount) {
    if (mapMount.querySelector(".map_area")) {
      initRepresentativesRegionMap();
      return;
    }

    document.addEventListener(
      "serwokontrol:mapReady",
      function () {
        initRepresentativesRegionMap();
      },
      { once: true },
    ...
    document.addEventListener(
      "serwokontrol:mapLoadError",
      function () {
        window.__serwokontrolRepresentativesMapInitialized = false;
      },
      { once: true },
    );
...
if (contactForm && formStatus && contactFormSubmit) {
  ...
  contactForm.addEventListener("submit", function (event) {
    ...
    fetch(contactForm.getAttribute("action") || "send-mail.php", {
...
(function () {
  function openFaqFromHash() {
    ...
    if (hash !== "#productsFaq" && hash !== "#contactFaq") return;
